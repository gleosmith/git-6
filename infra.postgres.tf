...
# ---------------------
resource "aws_lb_target_group" "aaaa" {
...
resource "aws_lb_listener" "aaaa" {
  load_balancer_arn = aws_lb.aaaa.arn
  port              = "8083"
  protocol          = "TCP"
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.aaaa.arn
  }
}
